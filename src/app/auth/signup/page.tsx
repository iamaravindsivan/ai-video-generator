...
type SignupState = "form" | "otp";

export default function SignupPage() {
  const [signupState, setSignupState] = useState<SignupState>("form");
  const [resendSuccess, setResendSuccess] = useState(false);
  const signupDataRef = useRef<OTPRequest | null>(null);
  const router = useRouter();
  const { setUser } = useAuth();

  const signupForm = useForm<OTPRequest>();
  ...
  const handleOtpSubmit = useCallback(
    (data: VerifyOTPData) => {
      ...
      const submitData = {
        ...
        otp: data.otp,
        fullName: signupDataRef.current!.fullName,
      };
  ...
  const handleResendOTP = useCallback(() => {
    if (signupDataRef.current) {
      otpForm.clearErrors();
      resendMutation.mutate(signupDataRef.current);
    }
  ...
  }, [signupForm, otpForm, signupMutation, otpMutation, resendMutation]);
                  ...
                  {signupForm.formState.errors.root && (
                    <div className="flex items-start text-red-600 text-sm p-3 bg-red-50 rounded border">
                      ...
                      <div>
                        ...
                        <div>{signupForm.formState.errors.root.message}</div>
                      </div>
                    </div>
                  )}
                </div>
              ...
              >
                ...
                <p className="text-muted-foreground text-sm text-center mb-2">
                  ...
                  <span className="font-medium">
                    {signupDataRef.current?.email}
                  </span>
                </p>
                <div className="w-full flex flex-col items-center">
                  <label className="mb-2 text-center w-full text-sm font-medium">
                    OTP Code
                  </label>
                  <InputOTP
                    value={otpValue}
